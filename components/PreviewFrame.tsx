...
type DeviceType = 'smallPhone' | 'mediumPhone' | 'largePhone';
...
const devices: Record<DeviceType, DeviceConfig> = {
  // Small phone - 720×1280 (scaled to fit preview)
  smallPhone: { 
    name: 'Small Phone (HD)', 
    width: 360, 
    height: 640, 
  ...
  // Large phone - 1440×3120 (QHD+ - scaled to fit preview)
  largePhone: { 
    name: 'Large Phone (QHD+)', 
    width: 480, 
    height: 1040, 
    notch: false, 
    type: 'android', 
...
export default function PreviewFrame({ 
  ...
  const [selectedDevice, setSelectedDevice] = useState<DeviceType>('mediumPhone');
  ...
  useEffect(() => {
    if (status === 'ready') {
      const message = `Viewing on ${currentDevice.name} (${currentDevice.width}×${currentDevice.height})`;
      console.log('📱', message);
    }
  }, [selectedDevice, status, currentDevice]);
