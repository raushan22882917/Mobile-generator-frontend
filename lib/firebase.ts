...
if (typeof window !== 'undefined') {
  ...
  if (isFirebaseConfigValid()) {
    try {
      if (getApps().length === 0) {
        ...
        if (typeof window !== 'undefined') {
          try {
            ...
            // Analytics might fail in some environments (e.g., localhost without proper setup)
      ...
      console.warn('Firebase configuration may be invalid. Please check your configuration.');
    }
  } else {
...
// Export auth and analytics (will be undefined on server-side or if config is invalid)
