...
export async function POST(request: NextRequest) {
  try {
    ...
    let response;
    try {
      response = await fetch(`${BACKEND_URL}/apply-template`, {
        method: 'POST',
        headers: {
        ...
        return NextResponse.json(
          {
            ...
            message: 'The request timed out after 2 minutes',
            suggestion: 'The backend operation is taking too long. Please try again or check the backend status.',
          },
          { status: 504 }
        );
      }
    ...
    if (!response.ok) {
      ...
      // Check for hung status in error response
      const isHung = errorData.result_code === 'hung' || 
                     errorData.error === 'hung' ||
                     errorData.error === 'RESULT_CODE_HUNG' ||
    ...
    // Check for hung status in successful response
    const isHung = data.result_code === 'hung' || 
                   data.result_code_hung === true ||
                   data.error === 'hung' ||
                   data.error === 'RESULT_CODE_HUNG' ||
                   data.message?.toLowerCase().includes('hung');
